...
#include <test_hunav/HuNavPlugin.h>
...
void HuNavPlugin::Load(gazebo::physics::WorldPtr _world, sdf::ElementPtr _sdf)
{
  ...
  if (hnav_->waitForGoal)
  {
    if (hnav_->goalTopic == "amcl_pose" || hnav_->goalTopic == "initialpose")
    {
      hnav_->pose_sub = hnav_->rosnode->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
          hnav_->goalTopic, 1, std::bind(&HuNavPluginPrivate::poseCallback, hnav_.get(), std::placeholders::_1));
    }
    else
    {
...
void HuNavPluginPrivate::poseCallback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{
  RCLCPP_INFO(rosnode->get_logger(), "\n\nPlugin! GOAL RECEIVED!!!!\n\n");
  goalReceived = true;
}
...
{
  const std::string robot_name = "omni_base";
  std::regex agent_regex("^agent[0-9]+$");
...
//        std::cout << "[INFO] 检测模型: " << modelObstacle->GetName()
//                          << " 位置: (" << obsPos.X() << ", " << obsPos.Y() << ", " << obsPos.Z() << ")" << std::endl;
        ...
        std::string obstacle_name = modelObstacle->GetName();
        if (obstacle_name == robot_name || obstacle_name == "goal" || std::regex_match(obstacle_name, agent_regex))
          continue;
        // **新增 Collision 级别的障碍物检测**
        for (size_t l = 0; l < modelObstacle->GetChildCount(); ++l)
        {
          gazebo::physics::BasePtr base = modelObstacle->GetChild(l);
          gazebo::physics::LinkPtr link = boost::dynamic_pointer_cast<gazebo::physics::Link>(base);
          if (!link)
            continue;

//          std::cout << "  [DEBUG] 发现 Link: " << link->GetName()
//                              << " 位置: (" << link->WorldPose().Pos().X() << ", "
//                              << link->WorldPose().Pos().Y() << ", "
//                              << link->WorldPose().Pos().Z() << ")" << std::endl;

          std::vector<gazebo::physics::CollisionPtr> collisions = link->GetCollisions();
          for (auto& collision : collisions)
          {
            if (!collision)
              continue;

            ignition::math::Vector3d obsPos = link->WorldPose().Pos();

//            std::cout << "    [DEBUG] 发现 Collision: " << collision->GetName()
//                                  << " in Link: " << link->GetName()
//                                  << " 位置: (" << obsPos.X() << ", " << obsPos.Y() << ", " << obsPos.Z() << ")" << std::endl;

            if (obsPos.X() != 0.0 && obsPos.Y() != 0.0)
            {
              ignition::math::Line3d act_obs_line(actorPos, obsPos);
              std::tuple<bool, double, ignition::math::Vector3d> obs_intersect = collision->BoundingBox().Intersect(act_obs_line);
                                                 ...
                                                 const hunav_msgs::msg::Agents& _agents)
{
  ...
  for (auto a : _agents.agents)
  {
    ...
    {
      yaw = normalizeAngle(currAngle + (diff * 0.5));  // 0.1, 0.01, 0.005
    }
