...
#include "test_hunav/WorldGenerator.hpp"
...
void WorldGenerator::readPluginParams()
{
  ...
  // try {
  //   pkg_shared_dir =
  //       ament_index_cpp::get_package_share_directory("test_hunav");

  // } catch (ament_index_cpp::PackageNotFoundError) {
  //   RCLCPP_ERROR(this->get_logger(),
  //                "Package test_hunav not found in dir: %s!!!",
...
bool WorldGenerator::processXML()
{
  ...
  const std::string BASE_PATH = "./src/test_hunav/media/models/";
...
//    // 获取 base_world_ 的目录路径
//    std::string base_path = base_world_.substr(0, base_world_.find_last_of("/\\") + 1);
//    // 生成皮肤文件的绝对路径
//    std::string skin_abs_path = base_path + skin_filename[a.skin];
//    // 设置 XML 节点的文本内容为绝对路径
//    pFilename->SetText(skin_abs_path.c_str());
    ...
    {
      pFilename1->SetText((BASE_PATH + animation_filename[0]).c_str());
    }
    else if (a.behavior.type == hunav_msgs::msg::AgentBehavior::BEH_IMPASSIVE)
    {
      pFilename1->SetText((BASE_PATH + animation_filename[1]).c_str());
    }
    else if (a.behavior.type == hunav_msgs::msg::AgentBehavior::BEH_SCARED)
    {
      pFilename1->SetText((BASE_PATH + animation_filename[5]).c_str());
    }
    ...
    if (a.behavior.type == hunav_msgs::msg::AgentBehavior::BEH_REGULAR)
    {
      pFilename2->SetText((BASE_PATH + animation_filename[0]).c_str());
    }
    else if (a.behavior.type == hunav_msgs::msg::AgentBehavior::BEH_IMPASSIVE)
    {
      pFilename2->SetText((BASE_PATH + animation_filename[1]).c_str());
    }
    else if (a.behavior.type == hunav_msgs::msg::AgentBehavior::BEH_SURPRISED)
    {
      pFilename2->SetText((BASE_PATH + animation_filename[2]).c_str());
    }
    else if (a.behavior.type == hunav_msgs::msg::AgentBehavior::BEH_THREATENING)
    {
      pFilename2->SetText((BASE_PATH + animation_filename[7]).c_str());
    }
    else if (a.behavior.type == hunav_msgs::msg::AgentBehavior::BEH_SCARED)
    {
      pFilename2->SetText((BASE_PATH + animation_filename[6]).c_str());
    }
    else if (a.behavior.type == hunav_msgs::msg::AgentBehavior::BEH_CURIOUS)
    {
      pFilename2->SetText((BASE_PATH + animation_filename[4]).c_str());
    }
